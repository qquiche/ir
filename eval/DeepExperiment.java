...
public class DeepExperiment {
  ...
  /**
   * The directory from which the pre-embedded documents come.
   */
  ...
  /**
   * The directory with an emdedding file for each query in queryFile
   * Query embedding files should be named Q1, Q2, .. one for each query
   * in the order they are in the queryFile.  The files will be lexicographically
   * sorted by name and this order should correspond to the order in the queryFile
   * so file numbers should have leading '0''s as needed to sort properly, i.e.
   * Q001, Q002, ... Q099, Q100.
   */
  public File queryVectorDir = null;
  ...
  /**
   * The DeepRetriever to use to process queries
   */
  ...
  /**
   * Create an DeepExperiment object for generating Recall/Precision curves
   *
   * @param corpusDir The directory of file embeddings
   * @param queryFile The file of query/relevant-docs pairs to evaluate.
   * @param queryVectorDir The directory of embeddings for the queries
   * @param outFile   File for output precision/recall data.
   * @param useCosine Use cosine similarity to rank retrievals rather than default Euclidian distance
   */
    public DeepExperiment(File corpusDir, File queryFile, File queryVectorDir,
			  File outFile, boolean useCosine)
      throws IOException {
	this.corpusDir = corpusDir;
	this.retriever = new DeepRetriever(corpusDir, useCosine);
	this.queryFile = queryFile;
	this.queryVectorDir = queryVectorDir;
  ...
  void processQueries() throws IOException {
    ...
    // Get the list of pre-embedded query files in order of queries in queryFile
    File[] queryFiles = queryVectorDir.listFiles();
    Arrays.sort(queryFiles,
		new Comparator() {
		    public int compare(Object o1, Object o2) {
			String str1 = ((File)o1).getName();
			String str2 = ((File)o2).getName();
   ...
   * Command format: "DeepExperiment [OPTION] [DIR] [QUERIES] [QUERYDIR] [OUTFILE]" where:
  ...
  public static void main(String[] args) throws IOException {
    ...
    String queryVectorFile = args[args.length - 2];
    String outFile = args[args.length - 1];
    boolean useCosine = false;
    if (args[0].equals("-cosine"))
	useCosine = true;
    DeepExperiment exper = new DeepExperiment(new File(corpusDir), new File(queryFile),
        new File(queryVectorFile), new File(outFile), useCosine);
    exper.makeRpCurve();
  }
}
